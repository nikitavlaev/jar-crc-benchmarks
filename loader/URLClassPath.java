...
public class URLClassPath {
     ...
     * Returns the original search path of URLs.
     */
    private String getURL(Loader loader) {
        String urlString = loader.getBaseURL().toString();
        int beginPos = urlString.startsWith("jar:")?4:0;
        int endPos = urlString.indexOf("!/");
        return urlString.substring(beginPos, endPos > -1 ? endPos : urlString.length());
    }
    ...
    public URL findResource(String name, boolean check) {
        // System.out.println("FindResource 1 " + name);
        Loader loader;
        int[] cache = getLookupCache(name);
        for (int i = 0; (loader = getNextLoader(cache, i)) != null; i++) {
            // System.out.println("FindResource loader " + loader);
            URL url = loader.findResource(name, check);
            if (url != null) {
                if (lookupCacheCreate) {
                    addResourceToLookupCacheForClassLoader(lookupCacheLoader, name, getURL(loader));
                }
                return url;
            }
        }
        if (lookupCacheCreate) {
            addResourceToLookupCacheForClassLoader(lookupCacheLoader, name, null);
        }
        return null;
    }
    ...
    private static boolean lookupCacheCreate
        = "true".equals(VM.getSavedProperty("sun.cds.createSharedLookupCache"));
    ...
    // create lookup cache
    private static native void setLookupCacheForClassLoader(ClassLoader loader,
                                                             String path);
    // add resource name to lookup cache
    private static native void addResourceToLookupCacheForClassLoader(ClassLoader loader,
                                                             String res,
                                                             String path);
     ...
     */
     private synchronized Loader getLoader(int index) {
        ...
        while (loaders.size() < index + 1) {
            ...
            if (lookupCacheCreate) {
              setLookupCacheForClassLoader(lookupCacheLoader, getURL(loader));
            }
        }
        if (DEBUG_LOOKUP_CACHE) {
    ...
    static class JarLoader extends Loader {
            ...
            throws IOException
        {
            ...
            // System.out.println("JarLoader " + url);
            csu = url;
            handler = jarHandler;
            lmap = loaderMap;
            this.acc = acc;
        ...
        static class ClassLoaderJarFile extends JarFile implements com.azul.crac.Resource {
            public ClassLoaderJarFile (File file) throws IOException {
                super(file);
            }
        ...
        private JarFile getJarFile(URL url) throws IOException {
            ...
            if (isOptimizable(url)) {
                ...
                ClassLoaderJarFile clJarFile = new ClassLoaderJarFile(new File(p.getPath()));
                com.azul.crac.Core.register(clJarFile);
        ...
        Resource checkResource(final String name, boolean check,
            ...
            return new Resource() {
                ...
                public String getName() { return name; }
                public int getCRC32() { return (int)entry.getCrc(); }
                public URL getURL() { return url; }
